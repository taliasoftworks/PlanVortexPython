"""The transport: httpx2, timeouts, retries and hooks.

It knows nothing about authentication — that is ``auth.py`` — and nothing about resources.
Everything the library sends to the API goes through here, the token request included, which is what
keeps the ``User-Agent``, the timeout and the backoff a single implementation rather than two that
drift apart.

The shapes it moves around — ``HttpRequest``, ``HttpResponse``, ``RetryConfig``, ``HttpHooks`` —
live in ``transport.py`` and are imported, not declared. They carry no I/O, so declaring them here
would mint a second copy of each in the generated twin, and two dataclasses with the same fields are
still two different types for a type checker.

THE THREE RETRY RULES, which are the ones that avoid a silent disaster:

1. **A 400 with a domain code is never retried.** Repeating "the text is too long" does not fix it,
   and in this API every domain error is a 400. Only 408, 429, 502, 503 and 504 are retried: the
   ones that mean "come back later".
2. **A POST that reached the server is never retried.** Publishing twice is worse than failing, and
   the API has no idempotency key yet. A POST is only repeated when the failure proves the request
   never left — no connection, no DNS — and never on a 5xx, which is the answer of a server that did
   receive it.
3. **A body that cannot be rewound is never repeated at all.** An upload streams from an open file,
   and ``httpx2`` reads it from wherever the pointer is: a second attempt over the same handle
   sends zero bytes. So a request either knows how to put itself back (``rewind``) or forbids the
   repeat outright (``repeatable=False``), including the pre-flight failures that rule 2 allows.

THIS FILE IS THE SOURCE OF ``http_sync.py``. Edit it and run ``uv run python
scripts/generate_sync.py``; CI regenerates and fails on a diff.
"""
