...
Gemelo sincrono de `src/planvortex/resources/organizations.py`.
...
class OrganizationsResource(Resource):
    ...
    def get(
        ...
        """
        organizacion: Organization = self._one(
    ...
    def remove(self, id_organization: str, *, timeout: float | None = None) -> None:
    ...
    def limits(self, id_organization: str, *, timeout: float | None = None) -> Limit:
        ...
        there at all.
        """
        limites: Limit = self._get(f"{self._path(id_organization)}/limits", timeout=timeout)
        return limites

    def use(self, id_organization: str, *, timeout: float | None = None) -> OrganizationUse:
        ...
        """
        organizacion = self.get(id_organization, get_use=True, timeout=timeout)
        uso: OrganizationUse = {}
    ...
    def delete_social_credentials(
        ...
        """
        organizacion: Organization = self._delete_one(
            self._credentials_path(id_organization, social_network), "organization", timeout=timeout
        )
        return organizacion

    def users(self, id_organization: str, *, timeout: float | None = None) -> list[OrganizationUser]:
        ...
        """
        cuerpo: Any = self._get(f"{self._path(id_organization)}/users", timeout=timeout)
