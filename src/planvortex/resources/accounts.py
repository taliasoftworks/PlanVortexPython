"""Social accounts connected to an organization.

TWO THINGS THAT ARE NOT GUESSABLE:

- **An app cannot CONNECT an account.** Connecting Instagram is an OAuth with a person in front of
  it, and app credentials are no good for that: :meth:`AsyncAccountsResource.connect_links`,
  :meth:`AsyncAccountsResource.connect` and :meth:`AsyncAccountsResource.enable` answer 519 when
  called with them. You issue a temporal token (``organizations.create_connect_token``), hand it to
  the person, and call these with a client authenticated by it: ``pv.as_temporal_token(token)``.
- **``error_code`` other than ``0`` is a broken account**, not a failure of this call. It stays in
  the listing with its data, but it neither publishes nor measures until somebody reconnects it.

THIS FILE IS THE SOURCE OF ``resources_sync/accounts.py``.
"""
...
class AsyncAccountsResource(AsyncResource):
    ...
    async def connect_links(
        ...
        * **Telegram has a link and still is not a redirect**: it opens a private chat with the
        ...
        """
        enlaces: list[ConnectLink] = await self._one(
    ...
    async def connect(
        ...
        **And they come back DISABLED**: they take no plan slot and publish nothing until
        ...
        cuerpo: AccountConnectResponse = await self._get(ruta, params, timeout=timeout)
    ...
    async def enable(
        ...
        **before** they pick the channel, not afterwards.
        """
        cuerpo: Any = await self._post(f"{self._path(id_organization, id_account)}/enable", timeout=timeout)
    ...
    async def list(
        ...
        *,
        ...
        """
        pagina: Page[Account] = await self._list(
    ...
    def aiterate(
        ...
        *,
        ...
        async def buscar(params: PageParams) -> Page[Account]:
    ...
    async def get(self, id_organization: str, id_account: str, *, timeout: float | None = None) -> Account:
        """One account's record."""
        cuenta: Account = await self._one(self._path(id_organization, id_account), "account", timeout=timeout)
        return cuenta
    ...
    async def update(
        ...
        """
        cuenta: Account = await self._put_one(
            self._path(id_organization, id_account), "account", body, timeout=timeout
        )
        return cuenta

    async def remove(self, id_organization: str, id_account: str, *, timeout: float | None = None) -> None:
        ...
        """
        await self._delete(self._path(id_organization, id_account), timeout=timeout)
    ...
    async def metrics(
        ...
        *,
        ...
        (§ Trampa P8).
        """
        metricas: AccountMetrics = await self._get(
    ...
    async def metric_list(
        ...
        """
        nombres: AccountMetricNames = await self._get(
    ...
    async def destinations(
        ...
        **Where they exist, the destination is REQUIRED**: what comes back here is what goes in
        ...
        destinos: builtins.list[Destination] = await self._one(
    ...
    async def destination(
        ...
        *,
        ...
        """
        destino: Destination = await self._one(
            f"{self._path(id_organization, id_account)}/destinations/"
    ...
    async def get_persistent_menu(
        ...
        """
        menu: PersistentMenu = await self._one(
    ...
    async def set_persistent_menu(
        ...
        *,
        ...
        """
        guardado: PersistentMenu = await self._post_one(
            f"{self._path(id_organization, id_account)}/persistent_menu",
            "persistent_menu",
